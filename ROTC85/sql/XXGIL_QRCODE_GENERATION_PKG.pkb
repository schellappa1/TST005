create or replace PACKAGE BODY xxgil_qrcode_generation_pkg 
AS
-- +=====================================================================================+
-- |
...
------------Step:1--Start: DEBUG FUNCS AND PROCS--------------------------

PROCEDURE request_log(
    p_text varchar2,
    p_level pls_integer default 1,
    p_new_line boolean default true) 
IS
-- +=====================================================================+
-- | Name               :     request_log
-- +=====================================================================+
...
PROCEDURE request_log_matrix(
    p_matrix t_column,
    p_level pls_integer default 1) IS
-- +=====================================================================+
-- | Name               :     request_log_matrix
-- | Called by          :  	masking_format_version, place_fm_in_matrix Procedure
-- +=====================================================================+
    lcQR varchar2(200);
...
BEGIN
    FOR t IN 1 .. p_matrix.count LOOP
        ...
        END LOOP;
        request_log(lcQR, p_level);
    END LOOP;
END request_log_matrix;
------------Step:1--End: DEBUG FUNCS AND PROCS--------------------------

------------Step:2--Start: UTILITY FUNCS AND PROCS--------------------------

FUNCTION bitor(p1 number, p2 number) RETURN number 
IS
-- +=====================================================================+
-- | Name               :     bitor
-- | Called by          :  	bitxor Function
-- +=====================================================================+
BEGIN
...
-- +=====================================================================+
-- | Name               :     bitxor
-- | Called by          :  	final_data_with_ec Function, 
-- |									fill_log_antilog Procedure
-- +=====================================================================+
BEGIN
  RETURN bitor(p1, p2) - bitand(p1, p2);
END bitxor;

FUNCTION bin2dec(binval varchar2) RETURN pls_integer 
IS
-- +=====================================================================+
-- | Name               :     bin2dec
-- | Called by          :  	final_data_with_ec Function
...
FUNCTION integer_2_binary(p_integer pls_integer) RETURN varchar2 
IS
-- +=====================================================================+
-- | Name               :     integer_2_binary
-- | Called by          :  	final_data_with_ec, encode_data, encode_data, char_count_indicator Function
-- +=====================================================================+
...
BEGIN
    ...
    RETURN lcBinary;
END integer_2_binary;

------------Step:2--End: UTILITY FUNCS AND PROCS--------------------------



------------Step:3--Start: INIT FUNCS AND PROCS--------------------------
PROCEDURE fill_log_antilog 
IS
-- +=====================================================================+
-- | Name               :     fill_log_antilog
-- | Called by          :  	generate_qr Procedure
-- +=====================================================================+
    lnVal pls_integer;
...
BEGIN
    FOR t IN 0 .. 255 LOOP
        ...
        request_log('Log for ' || t || ': ' || gprLog(t), 4);

    END LOOP;
END fill_log_antilog;

FUNCTION matrix_size RETURN pls_integer 
IS
-- +=====================================================================+
-- | Name               :     matrix_size
-- | Called by          :  	get_qrcode_bmp  Function
-- |									generate_qr_data,penalty_rules,masking_format_version, place_fm_in_matrix, init_matrix Procedure
-- +=====================================================================+
BEGIN
...
-- +=====================================================================+
    ...
    BEGIN
        ...
        if p_column - 2 < 1 or p_column + 2 > matrix_size or p_row - 2 < 1 or p_row + 2 > matrix_size then
            request_log('Alignment on ' || p_column || ', ' || p_row || ' is outside of matrix! Skipping...', 2);
            RETURN;
        end if;
        ...
        FOR t IN p_column - 2 .. p_column + 2 LOOP
            FOR p IN p_row - 2 .. p_row + 2 LOOP
                if gprMatrix(p)(t) is not null then
                    request_log('Alignment on ' || p_column || ', ' || p_row || ' overlaps with finder! Skipping...', 2);
                    RETURN;
                end if;
            END LOOP;
        END LOOP;
...
BEGIN
    request_log('Matrix size: ' || matrix_size || 'x' || matrix_size || ' modules', 2);

    --collection initialization
    gprMatrix.delete;

    gprMatrix.extend(matrix_size);
    FOR t IN 1 .. matrix_size LOOP
        gprMatrix(t) := t_row();
        gprMatrix(t).extend(matrix_size);
    END LOOP;


    --add Finder Patterns (3x)
    p_add_finder(1, 1);
    p_add_finder(matrix_size - 6, 1);
    p_add_finder(1, matrix_size - 6);
    ...
    FOR t IN 0 .. 7 LOOP
        ...
        --lower left
        gprMatrix(matrix_size - 7 + t)(1 + 7) := '2';
        gprMatrix(matrix_size - 7)(1 + t) := '2';

        --upper right
        gprMatrix(1 + t)(matrix_size - 7) := '2';
        gprMatrix(1 + 7)(matrix_size - 7 + t) := '2';
    END LOOP;
    ...
    FOR t IN 1 .. 8 LOOP
        if gprMatrix(matrix_size - t + 1)(9) is null then 
            gprMatrix(matrix_size - t + 1)(9) := '.';
        end if;

        if gprMatrix(9)(matrix_size - t + 1) is null then 
            gprMatrix(9)(matrix_size - t + 1) := '.';
        end if;
    END LOOP;
    ...
    if gpnVersion >= 7 then
        FOR t IN 1 .. 6 LOOP
            gprMatrix(matrix_size - 8)(t) := '*';
            gprMatrix(matrix_size - 9)(t) := '*';
            gprMatrix(matrix_size - 10)(t) := '*';

            gprMatrix(t)(matrix_size - 8) := '*';
            gprMatrix(t)(matrix_size - 9) := '*';
            gprMatrix(t)(matrix_size - 10) := '*';
...
------------Step:3--End: INIT FUNCS AND PROCS--------------------------


------------Step:4--Start: CALCULATION FUNC AND PROC-------------------

FUNCTION mode_name(p_type pls_integer) RETURN varchar2 
IS
-- +=====================================================================+
-- | Name               :     mode_name
-- | Description		   :     function returns QR code mode depending of data going to be encoded:
...
FUNCTION get_mode(p_data varchar2) RETURN pls_integer 
IS
-- +=====================================================================+
-- | Name               :     get_mode
-- | Description			: 		UTF-8 ascii values for kanji: 
...
BEGIN
    ...
    --debug
    request_log('mode for qr code: ' || mode_name(lnType) );

    RETURN lnType;
END get_mode;

FUNCTION err_cor_2_number(p_error_correction varchar2) 
RETURN number 
IS
-- +=====================================================================+
-- | Name               :     err_cor_2_number
-- | Called by          :  	get_version Function 
-- +=====================================================================+
...
-- +=====================================================================+
-- | Name               :     mode_indicator
-- | Description			:		4 different indicators which are encoded into QR code
-- | Called by          :  	encode_data Function 
-- +=====================================================================+
    lcMode varchar2(4);
BEGIN
    lcMode := 
    ...
    request_log('Mode indicator: ' || lcMode, 2);

    RETURN lcMode;
END mode_indicator;

FUNCTION char_count_indicator(p_data varchar2) 
RETURN varchar2 
IS
-- +=====================================================================+
-- | Name               :     char_count_indicator
-- | Description			: 		character count indicator - final length in bits depends of mode and version
-- | Called by          :  	encode_data Function 
-- +=====================================================================+
...
BEGIN
    ...
    lcIndicator := integer_2_binary(lnLength);

    request_log('CCI Binary: ' || lcIndicator, 2);
    ...
    request_log('Character Count Indicator length: ' || lnCCILength, 2);


    lcIndicator := lpad(lcIndicator, lnCCILength, '0');

    request_log('Character Count Indicator: ' || lcIndicator, 2);


    RETURN lcIndicator;
END char_count_indicator;
...
-- +=====================================================================+
-- | Name               :     get_version
-- | Description			: 		function determins and returns a QR code version (size), which depends of 3 values:
...
BEGIN
    ...
    */
    ...
    lnPosition := (err_cor_2_number(p_error_correction) - 1) * 4 + gpnMode;

    request_log('length: ' || lnLength, 2);

    FOR t IN 1 .. 40 LOOP
        request_log(lrValues(t), 4);

        SELECT to_number(column_value) a
        BULK COLLECT INTO lrData
        FROM XMLTABLE(lrValues(t));

        request_log('Value on position ' || lnPosition || ': ' || lrData(lnPosition), 2);
...
BEGIN
    ...
    lcData := mode_indicator || char_count_indicator(p_data);
    ...
    if gpnMode = cNumericMode then
        LOOP
            ...
            if length(lcSub) = 3 then
                lcData := lcData || lpad(integer_2_binary( to_number(lcSub) ), 10, '0');
            elsif length(lcSub) = 2 then
                lcData := lcData || lpad(integer_2_binary( to_number(lcSub) ), 7, '0');
            elsif length(lcSub) = 1 then
                lcData := lcData || lpad(integer_2_binary( to_number(lcSub) ), 4, '0');
            end if;
        ...
        LOOP
            ...
            if length(lcSub) = 2 then
                lcData := lcData || lpad(integer_2_binary( f_get_code(substr(lcSub, 1, 1)) * 45 + f_get_code(substr(lcSub, 2, 1)) ), 11, '0');
            elsif length(lcSub) = 1 then
                lcData := lcData || lpad(integer_2_binary( f_get_code(lcSub) ), 6, '0');
            end if;
        ...
        FOR t IN 1 .. length(p_data) LOOP
            lcChar := substr(p_data, t, 1);
            lcData := lcData || lpad(integer_2_binary( ascii(lcChar) ), 8 * lengthb(lcChar), '0');
    ...
    request_log('Data without right padding (number of bits ' || length(lcData) || '): ' || lcData, 2);

    --terminator zeros
    lnReqNumOfBits := gprErrCorInfo(gpnVersion || '-' || p_error_correction).total_no_of_data_cw * 8;
    request_log('Required number of bits: ' || lnReqNumOfBits, 2);

    if lnReqNumOfBits - lengthb(p_data) >= 4 then
        lcData := lcData || '0000';
        request_log('Terminator zeros: 0000', 2);
    else
        lcData := rpad(lcData, lnReqNumOfBits - lengthb(p_data), '0');
        request_log('Terminator zeros: ' || rpad(null, lnReqNumOfBits - lengthb(p_data), '0'), 2);
    end if;
    ...
    --additional right padding with 0 to reach a string length multiple of 8
     LOOP
        ...
        lcData := lcData || '0';
        request_log('Additional right padding zeros: 0', 2);
    END LOOP;
    ...
    --final filling of data with 11101100 and 00010001 alternatively until full length is reached
     LOOP
        ...
        request_log('Additional padding with: 11101100', 2);
        EXIT WHEN length(lcData) = lnReqNumOfBits;
        lcData := lcData || '00010001';
        request_log('Additional padding with: 00010001', 2);
    END LOOP;


    --debug
    request_log('Data (number of bits ' || length(lcData) || '): ' || lcData);
...
FUNCTION final_data_with_ec(
    p_encoded_data varchar2,
    p_error_correction varchar2) 
RETURN varchar2 
IS
-- +=====================================================================+
-- | Name               :     final_data_with_ec
-- | Called by          :  	generate_qr Procedure 
-- +=====================================================================+
    ...
    PROCEDURE request_log_collection(
    ...
    BEGIN
        ...
        END LOOP;
        request_log(rtrim(lcDebug, ', '), p_level);
    END;
    ...
    BEGIN
        ...
        FOR t IN 1 .. p_mess_poly.count LOOP
            ...
            if lrEC(t) > 0 then
                ...
                request_log('Lead Term of the Message Polynomial: ' || lrEC(t) || ' (antiLog ' || lnAnti || ')', 2);
                ...
                request_log('skipping this step and discarding next lead term 0...', 2);
            end if;
...
BEGIN
    ...
    request_log('Version and EC Level: ' || lcVersionEC, 2);
    ...
    request_log('Number of groups: ' || lnGroups, 2);
    FOR gr IN 1 .. lnGroups LOOP
        request_log('Group ' || gr || ' - number of blocks in group: ' || f_blocks_in_group(gr), 2);
        request_log('Group ' || gr || ' - number of codewords in each block: ' || f_cw_in_group(gr), 2);
    END LOOP;
    ...
    FOR gr IN 1 .. lnGroups LOOP
        FOR blk IN 1 .. f_blocks_in_group(gr) LOOP
            ...
            request_log('Group ' || gr || ', block ' || blk || ' - message polynomial:', 2); 
            ...
            FOR cw IN 1 .. f_cw_in_group(gr) LOOP
                ...
                request_log('Codeword ' || cw || ': ' || lcCW || ' (' || lnCW || ')', 2);
        ...
        FOR gr IN 1 .. lnGroups LOOP
            FOR blk IN 1 .. f_blocks_in_group(gr) LOOP
                ...
                if lrBlock('G' || gr || 'B' || blk).cw.count >= cnt then
                    lcResult := lcResult || lpad(integer_2_binary( lrBlock('G' || gr || 'B' || blk).cw(cnt) ), 8, '0');
    ...
    FOR cnt IN 1 .. gprErrCorInfo(lcVersionEC).ec_cw_per_block LOOP
        FOR gr IN 1 .. lnGroups LOOP
            FOR blk IN 1 .. f_blocks_in_group(gr) LOOP
                lcResult := lcResult || lpad(integer_2_binary( lrBlock('G' || gr || 'B' || blk || 'EC').cw(cnt) ), 8, '0');
    ...
    lnReminderBits := 
        CASE
            ...
            ELSE 0
            END;
    request_log('Reminder bits: ' || lnReminderBits, 2);

    if lnReminderBits > 0 then
        lcResult := rpad(lcResult, length(lcResult) + lnReminderBits, '0');
    end if;

    request_log('Final message (length ' || length(lcResult) || '): ' || lcResult);


    RETURN lcResult;
END final_data_with_ec;



PROCEDURE place_fm_in_matrix(lcData varchar2) 
IS
-- +=====================================================================+
-- | Name               :     place_fm_in_matrix
-- | Called by          :  	generate_qr Procedure 
-- +=====================================================================+
...
BEGIN
    ...
    --debug filled matrix
    request_log('Matrix filled with final message (modules marked with dots are for format data; marked with stars are for version - those data will be filled in masking step):', 2);
    request_log_matrix(gprMatrix, 2);
END place_fm_in_matrix;


PROCEDURE masking_format_version(p_error_correction varchar2) 
IS
-- +=====================================================================+
-- | Name               :     masking_format_version
-- | Called by          :  	generate_qr Procedure 
-- +=====================================================================+
    ...
    BEGIN
        ...
        request_log('Format for error correction ' || p_error_correction || ' and masking ' || p_mask || ': ' || lcFormat, 3);
        ...
        p_set_module(matrix_size, 9, 1, 'F');
        p_set_module(matrix_size - 1, 9, 2, 'F');
        p_set_module(matrix_size - 2, 9, 3, 'F');
        p_set_module(matrix_size - 3, 9, 4, 'F');
        p_set_module(matrix_size - 4, 9, 5, 'F');
        p_set_module(matrix_size - 5, 9, 6, 'F');
        p_set_module(matrix_size - 6, 9, 7, 'F');

        p_set_module(9, matrix_size - 7, 8, 'F');
        p_set_module(9, matrix_size - 6, 9, 'F');
        p_set_module(9, matrix_size - 5, 10, 'F');
        p_set_module(9, matrix_size - 4, 11, 'F');
        p_set_module(9, matrix_size - 3, 12, 'F');
        p_set_module(9, matrix_size - 2, 13, 'F');
        p_set_module(9, matrix_size - 1, 14, 'F');
        p_set_module(9, matrix_size, 15, 'F');


        --put version in matrix (for 7 and higher)
        if gpnVersion >= 7 then
            lcVersion := lrVersion(gpnVersion);
            request_log('Version (' || gpnVersion || ') for matrix: ' || lcVersion, 3);
            ...
            FOR c IN REVERSE 1 .. 6 LOOP
                FOR r IN 8 .. 10 LOOP
                    --bottom left 
                    p_set_module(matrix_size - r, c, lnCounter, 'V');

                    --top right
                    p_set_module(c, matrix_size - r, lnCounter, 'V');
            ...
            request_log('Version (' || gpnVersion || ') is not needed for matrix', 3);
        end if;
...
BEGIN
    ...
    FOR t IN 0 .. 7 LOOP
        ...
        FOR r IN 1 .. matrix_size LOOP
            FOR c IN 1 .. matrix_size LOOP
        ...
        request_log('Mask pattern ' || t || ':', 3);
        request_log_matrix(gprMasking(t), 3);
        request_log('------------------------------------------------------------------------------------------', 3);
...
END masking_format_version;


PROCEDURE penalty_rules(p_masking_out pls_integer default null) 
IS
-- +=====================================================================+
-- | Name               :     penalty_rules
-- | Called by          :  	generate_qr Procedure 
...
BEGIN
    ...
    FOR t IN 0 .. 7 LOOP
        ...
        FOR r IN 1 .. matrix_size LOOP  --horizontal direction
            lnCounter := 1;
            FOR c IN 2 .. matrix_size LOOP
                if gprMasking(t)(r)(c) = gprMasking(t)(r)(c - 1) then
                    lnCounter := lnCounter + 1;
                end if;
                if gprMasking(t)(r)(c) <> gprMasking(t)(r)(c - 1) or c = matrix_size then
                    if lnCounter >= 5 then
        ...
        lnDebugTemp := lnRule1;
        request_log('Masking ' || t || ', rule 1 - horizontal direction: ' || lnRule1, 2);

        FOR c IN 1 .. matrix_size LOOP  --vertical direction
            lnCounter := 1;
            FOR r IN 2 .. matrix_size LOOP
                if gprMasking(t)(r)(c) = gprMasking(t)(r - 1)(c) then
                    lnCounter := lnCounter + 1;
                end if;
                if gprMasking(t)(r)(c) <> gprMasking(t)(r - 1)(c) or r = matrix_size then
                    if lnCounter >= 5 then
                        lnRule1 := lnRule1 + 3 + (lnCounter - 5);  --if 5 modules are the same color then penalty is 3 plus 1 for every additional module (4 for 6 modules...)
                    end if;
                    lnCounter := 1;
        ...
        request_log('Masking ' || t || ', rule 1 - vertical direction: ' || (lnRule1 - lnDebugTemp), 2);
        request_log('Masking ' || t || ', rule 1 - sum: ' || lnRule1, 2);
        request_log(' ', 2);
        ...
        FOR r IN 1 .. matrix_size - 1 LOOP
            FOR c IN 1 .. matrix_size - 1 LOOP
                if 
        ...
        request_log('Masking ' || t || ', rule 2 - sum: ' || lnRule2, 2);
        request_log(' ', 2);
        ...
        FOR r IN 1 .. matrix_size LOOP  --horizontal direction
            FOR c IN 1 .. matrix_size - 10 LOOP
                lcPattern := null;
                FOR p IN 0 .. 10 LOOP
                ...
                if lcPattern in ('10111010000', '00001011101') then
                    request_log(lcPattern || ' (horizontal) found in row ' || r || ' and column ' || c, 3);
                    lnRule3 := lnRule3 + 40;
                end if;
            END LOOP;
        END LOOP;
        ...
        lnDebugTemp := lnRule3;
        request_log('Masking ' || t || ', rule 3 - horizontal direction: ' || lnRule3, 2);

        FOR c IN 1 .. matrix_size LOOP  --horizontal direction
            FOR r IN 1 .. matrix_size - 10 LOOP
                lcPattern := null;
                FOR p IN 0 .. 10 LOOP
                ...
                if lcPattern in ('10111010000', '00001011101') then
                    request_log(lcPattern || ' (vertical) found in row ' || r || ' and column ' || c, 3);
                    lnRule3 := lnRule3 + 40;
                end if;
            END LOOP;
        END LOOP;

        request_log('Masking ' || t || ', rule 3 - vertical direction: ' || (lnRule3 - lnDebugTemp), 2);
        request_log('Masking ' || t || ', rule 3 - sum: ' || lnRule3, 2);
        request_log(' ', 2);
        ...
        lnPercent := trim(lnCounter * 100 / (matrix_size * matrix_size));  --calculate percent of dark modules

        request_log('Masking ' || t || ', rule 4 - total number of modules: ' || (matrix_size * matrix_size), 2);
        request_log('Masking ' || t || ', rule 4 - dark modules: ' || lnCounter, 2);
        request_log('Masking ' || t || ', rule 4 - % of dark modules: ' || lnPercent, 2);
        ...
        request_log('Masking ' || t || ', rule 4 - value: ' || lnRule4, 2);
        request_log(' ', 2);


        request_log('Masking ' || t || ', final penalty value: ' || (lnRule1 + lnRule2 + lnRule3 + lnRule4), 2);
        request_log('-----------------------------------------', 2);
    ...
    request_log('Masking ' || lnChosenMasking || ' has lowest penalty of ' || lnLowestPenalty || ' and will be used for QR code matrix.', 2);
...
PROCEDURE generate_qr(
    p_data varchar2,
    p_error_correction varchar2,
    request_log boolean default false,
    request_log_level pls_integer default 1,
    p_masking_out pls_integer default null
    ) IS
-- +=====================================================================+
-- | Name               :     generate_qr
-- | Called by          :  	generate_qr_data Procedure 
-- +=====================================================================+
    lcData varchar2(32000);
...
BEGIN
    ...
    gpbDebug := request_log;
    gpnDebugLevel := request_log_level;

    gpnMode := get_mode(p_data);
    gpnVersion := get_version(p_data, p_error_correction);
...
-- +=====================================================================+
-- | Name               :     generate_qr_data
-- | Called by          :  	qr_as_bmp Function  
-- +=====================================================================+
BEGIN
    --generate matrix
    generate_qr(
    ...
    --matrix size
    p_matrix_size := matrix_size;

END generate_qr_data;
...
FUNCTION unsigned_short(s in pls_integer) 
RETURN raw IS
-- +=====================================================================+
-- | Name               :     unsigned_short
-- | Called by          :  	get_qrcode_bmp Function  
...
FUNCTION unsigned_int(i in pls_integer) 
RETURN raw 
IS 
-- +=====================================================================+
-- | Name               :     unsigned_int
...
FUNCTION get_qrcode_bmp(
    p_data varchar2,  
    p_error_correction varchar2,
    p_margines varchar2 default 'N' 
    ) RETURN blob IS
-- +=====================================================================+
-- | Name               :     get_qrcode_bmp
-- | Description			: 		Function return black and white BMP image with QR code
-- |									Modules are 8x8 pixels large
--	| Parameter				: 		p_data - data that is going to be encoded into QR code
...
-- | Called by          :  	get_qrcode_clob Function  
-- +=====================================================================+
    lcQR varchar2(32727);
...
FUNCTION bmp2jpg(p_bmp_image blob) 
RETURN blob 
AS
-- +=====================================================================+
-- | Name               :	bmp2jpg
-- | Description		:	Function convert the format from bmp to JPG for XMLP Template
-- | Called by          :  	PLM Reports 
-- +=====================================================================+
...
IS
-- +=====================================================================+
-- | Name               :   get_qrcode_clob
-- | Description		: 	Function return the QR Code in CLOB format. 
-- |						it can be used in XMLP Reports.
-- | Called by          :  	PLM Reports 
-- +=====================================================================+
   p_source   BLOB;
   v_result   CLOB;
BEGIN
	p_source := bmp2jpg(get_qrcode_bmp(
			p_data => p_text||CHR(0),
